        ...
        {#each players as player, i}
            <tr
                ...
                {#if gameData.Variant === 'steel'}
                    {#if gameData.ThrowRound >= 2}
                        <td
                            ...
                            {gameData.Podium.includes(player.UID) ? 'Place ' + (gameData.Podium.indexOf(player.UID) + 1) : hitOrder[gameData.ThrowRound - 2]}
                        </td>
                    {:else}
                        <td
                            class="px-3 border-r border-l border-dashed border-opacity-10">
                ...
                {:else}
                    <td
                        class="px-3 border-r border-l border-dashed border-opacity-10">
                        {gameData.Podium.includes(player.UID) ? 'Place ' + (gameData.Podium.indexOf(player.UID) + 1) : hitOrder[gameData.ThrowRound - 1]}
                    </td>
                {/if}
